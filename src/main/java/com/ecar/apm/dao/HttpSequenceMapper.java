...
public interface HttpSequenceMapper {
	...
	@Insert("INSERT INTO http_sequence(guid,`group`,`type`,`name`,remark,jobName,frequency, isAsyn)  VALUES("
			+"#{httpSequence.guid},#{httpSequence.group},#{httpSequence.type},"
			+ "#{httpSequence.name},#{httpSequence.remark},#{httpSequence.jobName},#{httpSequence.frequency}, #{httpSequence.isAsyn})")
	...
	@Update("update http_sequence t set t.`group`=#{httpSequence.group},t.`type`=#{httpSequence.type},"
			+ "t.`name`=#{httpSequence.name},t.remark=#{httpSequence.remark},t.frequency=#{httpSequence.frequency}, t.isAsyn=#{httpSequence.isAsyn} WHERE t.`guid`= #{httpSequence.guid}")
